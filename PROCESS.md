...
Markdown files use TOML frontmatter with `+++` delimiters (Zola convention). Dates are unquoted TOML native dates (YYYY-MM-DD). Rust source files use `//` comment lines with the same key-value format.
